...
%% Decoding each group...
disp('Choose your data file...')
[fn, pn] = uigetfile('*.mat');
load([pn, fn])
timeseries = data.spikes;

disp('Choose your floating file...')
[fn, pn] = uigetfile('*.mat');
load([pn, fn])
heading = floating.alpha;

segment_length = inputdlg('Switch segment dur (s): ');
segment_length = str2double(segment_length{1});
...
tuning = tuning(hda(1).is_head_direction, :);
...
% all of ours analyses assume the binwidth of 3 doesn't change...
...
switch_idx = 1:switch_frames:length(timeseries)-50;
...
idx_mat(end+1, :) = switch_idx(end):length(timeseries)-50;
