classdef NeurotarPreProcessor < handle
	...
	methods
		...
		function [data, floating] = processData(obj, averageWindow)    
			...
			if obj.floating.is_old
				time = obj.extractTimeOLD(obj.floating);
			else
			...
			obj.floating.heading = obj.cleanAlpha(obj.floating.alpha);
		...
		function heading = cleanAlpha(obj, heading, threshold)
			if nargin < 3 || isempty(threshold)
				threshold = 1;
			end

			changes = diff(heading);
			heading(abs(changes) > threshold*std(changes)) = NaN;
		end
		...
		function equalizeFs(obj, time, isUniform)
			if isUniform
				...
				if obj.forceTimeLock
					fprintf('Forced to lock to stimulus time... \n')
				else
					fprintf('Your sampling rate is too uneven, subsampling neurotar data to match DFF... \n')
				end
				twoP_sampled_times = 1/obj.data.frameRate:1/obj.data.frameRate:(size(obj.data.raw_F, 2)/ obj.data.frameRate);
