classdef LightDarkAnalyzer < handle
    ...
    methods
        ...
        function classifyResponses(obj, check_flag)
            ...
            % Adding this new "doubling procedure', to increase the "repeats"
            ...
            for c = 1:size(light_tuning, 1)
                ...
                light = (circshift(light_tuning(c, :), size(light_tuning, 2)/4 - round(coeffs(3))));
                
                %coeffs = obj.fitDoubleGaussian(dark_data(c, :), modelfun);
                
                dark = (circshift(dark_tuning(c, :), size(light_tuning, 2)/4 - round(coeffs(3))));
